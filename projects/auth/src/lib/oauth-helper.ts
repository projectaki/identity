...
export const createAuthUrl = (authConfig: AuthConfig, codeChallenge?: string, state?: string) => {
  ...
  const authUrl = `${endPoint}?response_type=${responseType}&client_id=${clientId}&redirect_uri=${redirectUrl}${
    state ? `&state=${state}` : ''
  }${audience ? `&audience=${audience}` : ''}${codeChallenge ? `&code_challenge=${codeChallenge}` : ''}${
    codeChallenge ? `&code_challenge_method=S256` : ''
  }`;

  function getAuthorizeUrlParameters(authConfig: AuthConfig): AuthorizeUrlParams {
    const { responseType, clientId, redirectUrl, audience, issuer, useDiscovery, authorizeEndpoint } = authConfig;
...
export const createTokenRequestBody = (authConfig: AuthConfig, code: string, codeVerifier?: string) => {
  ...
  const body = `grant_type=${grantType}&code=${code}&redirect_uri=${authConfig.redirectUrl}&client_id=${authConfig.clientId}&code_verifier=${codeVerifier}`;
  return body;
};
